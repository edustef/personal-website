...
export const getAvailability = query({
	...
	handler: async (ctx, args) => {
		...
		// 2. Get active users from Presence component (if available) + random inflation
		const realActive = 0;
		try {
			// Attempt to query presence for the room (date)
		} catch (e) {
			// ignore
		}

		const randomInflation = Math.floor(Math.random() * (29 - 13 + 1)) + 13;
		const totalActive = realActive + randomInflation;
