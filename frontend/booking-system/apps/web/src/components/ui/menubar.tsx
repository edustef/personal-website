...
function Menubar({ className, ...props }: MenubarPrimitive.Props) {
  return (
    <MenubarPrimitive
      data-slot="menubar"
      className={cn("bg-background h-8 gap-0.5 rounded-md border p-1 flex items-center", className)}
      {...props}
    />
  )
}
...
function MenubarSubTrigger({
  ...
  return (
    <DropdownMenuSubTrigger
      ...
      data-inset={inset}
      className={cn("focus:bg-accent focus:text-accent-foreground data-open:bg-accent data-open:text-accent-foreground gap-2 rounded-md px-2 py-2 text-xs data-[inset]:pl-8 [&_svg:not([class*='size-'])]:size-4", className)}
      {...props}
    />
  )
}
...
function MenubarSubContent({
  ...
  return (
    <DropdownMenuSubContent
      ...
      className={cn("bg-popover text-popover-foreground data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 min-w-32 rounded-md shadow-lg ring-1 duration-100", className)}
      {...props}
    />
  )
}
